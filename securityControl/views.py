from django.shortcuts import get_object_or_404, render, redirect
from securityControl.models import Page, Taseron, Ziyaretci, havuz_arac, yuk_arac
from securityControl.forms.forms import HavuzAracForm, TaseronForm, ZiyaretciForm, yukAracForm, CalisanForm
from django.contrib.auth.decorators import login_required
# Create your views here.

@login_required (login_url='/login/')
def page_views(request, page_slug=None, location=None):
    page = None
    sc_pages = None
    title = None
    if page_slug == None:
        title = "Ana Sayfa"
        sc_pages = Page.objects.filter(is_active=True)
        if sc_pages:
            sc_pages = sc_pages
        else:
            sc_pages = None
    if page_slug:
        if page_slug == "ziyaretci":
            return ziyaretci_views(request, page_slug, location)
        if page_slug == "taseron":
            return taseron_views(request, page_slug, location)
        if page_slug == "yuk-arac":
            return yuk_arac_views(request, page_slug, location)
        if page_slug == "havuz-arac":
            return havuz_arac_views(request, page_slug)
        else:
            page = get_object_or_404(Page, slug=page_slug, is_active=True)
            title = page.title

    context = dict(
        page_slug=page_slug,
        sc_pages=sc_pages,
        page_title=title,
        page=page,
    )
    return render(request, "sc_pages/sec-home-views.html", context)


def ziyaretci_views(request, page_slug, location=None):
    ziyaretciler = Ziyaretci.objects.all()
    if ziyaretciler:
        ziyaretciler = ziyaretciler
        page_slug = page_slug
    else:
        ziyaretciler = None
        page_slug = page_slug
    if request.method == "POST":
        ziyaretci_form = ZiyaretciForm(request.POST)
        if ziyaretci_form.is_valid():
            ziyaretci_form.save()
            if location:
                return redirect("sc:sec-home-views", page_slug=location)
            else:
                return redirect("sc:sec-home-views", page_slug=page_slug)

    else:
        ziyaretci_form = ZiyaretciForm()
    sc_pages = None
    title = "Ziyaretçi Kayıt"
    context = dict(
        page_title=title,
        sc_pages=sc_pages,
        ziyaretci_form=ziyaretci_form,
        ziyaretciler=ziyaretciler,
        page_slug=page_slug,
    )
    return render(request, "sc_pages/sec-home-views.html", context)


def taseron_views(request, page_slug, location=None):
    ziyaretci_form = ZiyaretciForm()
    ziyaretciler = Ziyaretci.objects.all()
    taseronlar = Taseron.objects.all()
    if taseronlar:
        taseronlar = taseronlar
        page_slug = page_slug
    else:
        taseronlar = None
        page_slug = page_slug
    if request.method == "POST":
        taseron_form = TaseronForm(request.POST)
        if taseron_form.is_valid():
            taseron_form.save()
            if location:
                return redirect("sc:sec-home-views", page_slug=location)
            else:
                return redirect("sc:sec-home-views", page_slug=page_slug)

    else:
        taseron_form = TaseronForm()
    context = dict(
        taseron_form=taseron_form,
        page_slug=page_slug,
        taseronlar=taseronlar,
        ziyaretci_form=ziyaretci_form,
        ziyaretciler=ziyaretciler,
    )
    return render(request, "sc_pages/sec-home-views.html", context)


def yuk_arac_views(request, page_slug, location=None):
    ziyaretci_form = ZiyaretciForm()
    ziyaretciler = Ziyaretci.objects.all()
    ziyaretciler = Ziyaretci.objects.all()
    yuk_araclari = yuk_arac.objects.filter(is_inside=True)
    if yuk_araclari:
        yuk_araclari = yuk_araclari
    else:
        yuk_araclari = None

    if request.method == "POST":
        yuk_arac_form = yukAracForm(request.POST)
        if yuk_arac_form.is_valid():
            yuk_arac_form.save()
            if location:
                return redirect("sc:sec-home-views", page_slug=location)
            else:
                return redirect("sc:sec-home-views", page_slug=page_slug)

        else:
            yuk_arac_form = yukAracForm()

    else:
        yuk_arac_form = yukAracForm()

    context = dict(
        yuk_arac_form=yuk_arac_form,
        yuk_araclari=yuk_araclari,
        page_slug=page_slug,
        ziyaretci_form=ziyaretci_form,
        ziyaretciler=ziyaretciler,
    )
    return render(request, "sc_pages/sec-home-views.html", context)


def havuz_arac_views(request, page_slug):
    havuz_araclari = havuz_arac.objects.all()
    if havuz_araclari:
        havuz_araclari = havuz_araclari
    else:
        havuz_araclari = None
    if request.method == "POST":
        calisan_form = CalisanForm(request.POST)
        if calisan_form.is_valid():
            calisan_form.save()
            return redirect("sc:sec-home-views", page_slug=page_slug)
        havuz_arac_form = HavuzAracForm(request.POST)
        if havuz_arac_form.is_valid():
            havuz_arac_form.save()
            return redirect("sc:sec-home-views", page_slug=page_slug)
    else:
        havuz_arac_form = HavuzAracForm()
        calisan_form = CalisanForm()
    context = dict(
        havuz_arac_form=havuz_arac_form,
        page_slug=page_slug,
        havuz_araclari=havuz_araclari,
        calisan_form=calisan_form,
    )
    return render(request, "sc_pages/sec-home-views.html", context)
